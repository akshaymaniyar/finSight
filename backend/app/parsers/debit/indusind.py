...
class IndusIndParser(BaseBankParser):
    ...
    def parse_email(
        ...
        # Pattern 1: "Rs.{amt} has been debited from your IndusInd Bank A/c {acct}"
        debit_match = re.search(
            r"(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+has\s+been\s+debited\s+from\s+(?:your\s+)?(?:IndusInd\s+Bank\s+)?(?:A/[Cc]|Account)\s*([*Xx]*\d{4,6})",
            text,
            re.IGNORECASE,
        )
        if debit_match:
        ...
        # Pattern 2: "Your A/c {acct} is debited by Rs {amt}"
        debit_alt = re.search(
            r"(?:Your\s+)?(?:A/[Cc]|Account)\s*([*Xx]*\d{4,6})\s+(?:is|has been)\s+debited\s+(?:by|with|for)\s+(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)",
            text,
            re.IGNORECASE,
        )
        if debit_alt:
        ...
        credit_match = re.search(
            r"(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+has\s+been\s+credited\s+to\s+(?:your\s+)?(?:IndusInd\s+Bank\s+)?(?:A/[Cc]|Account)\s*([*Xx]*\d{4,6})",
            text,
            re.IGNORECASE,
        )
        if credit_match:
    ...
    def parse_pdf(
        ...
        """Parse IndusInd Bank bank account PDF statement."""
