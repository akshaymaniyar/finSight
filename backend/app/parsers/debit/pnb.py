...
class PNBParser(BaseBankParser):
    ...
    sender_emails = ["alerts@pnb.co.in"]
    ...
    def parse_email(
        ...
        # Pattern 1: "Your PNB A/c {acct} debited Rs.{amt}"
        debit_match = re.search(
            r"(?:Your\s+)?(?:PNB\s+)?(?:A/[Cc]|Account)\s*([*Xx]*\d{4,6})\s+(?:has been\s+)?debited\s+(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)",
            text,
            re.IGNORECASE,
        )
        if debit_match:
        ...
        # Pattern 2: "Rs {amt} debited from your PNB A/c {acct}"
        debit_alt = re.search(
            r"(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+(?:has been\s+)?debited\s+from\s+(?:your\s+)?(?:PNB\s+)?(?:A/[Cc]|Account)\s*([*Xx]*\d{4,6})",
            text,
            re.IGNORECASE,
        )
        if debit_alt:
        ...
        credit_match = re.search(
            r"(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+(?:has been\s+)?credited\s+to\s+(?:your\s+)?(?:PNB\s+)?(?:A/[Cc]|Account)\s*([*Xx]*\d{4,6})",
            text,
            re.IGNORECASE,
        )
        if credit_match:
    ...
    def parse_pdf(
        ...
        """Parse Punjab National Bank bank account PDF statement."""
