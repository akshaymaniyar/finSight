...
class IndianBankParser(BaseBankParser):
    ...
    sender_emails = ["alerts@indianbank.co.in"]
    ...
    def parse_email(
        ...
        # Pattern 1: "Your Indian Bank A/c {acct} is debited for Rs.{amt}"
        debit_match = re.search(
            r"(?:Your\s+)?(?:Indian\s+Bank\s+)?(?:A/[Cc]|Account)\s*([*Xx]*\d{4,6})\s+(?:is|has been)\s+debited\s+(?:for|with|by)\s+(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)",
            text,
            re.IGNORECASE,
        )
        if debit_match:
        ...
        debit_alt = re.search(
            r"(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+(?:has been\s+)?debited\s+from\s+(?:your\s+)?(?:Indian\s+Bank\s+)?(?:A/[Cc]|Account)\s*([*Xx]*\d{4,6})",
            text,
            re.IGNORECASE,
        )
        if debit_alt:
        ...
        credit_match = re.search(
            r"(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+(?:has been\s+)?credited\s+to\s+(?:your\s+)?(?:Indian\s+Bank\s+)?(?:A/[Cc]|Account)\s*([*Xx]*\d{4,6})",
            text,
            re.IGNORECASE,
        )
        if credit_match:
    ...
    def parse_pdf(
        ...
        """Parse Indian Bank bank account PDF statement."""
