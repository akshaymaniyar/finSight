...
class AxisParser(BaseBankParser):
    ...
    def parse_email(
        ...
        # Pattern 1: "Rs.{amt} has been debited from A/C No. {acct}"
        debit_match = re.search(
            r"(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+has\s+been\s+debited\s+from\s+(?:A/C|a/c|Account)\s*(?:No\.?\s*)?([*Xx]*\d{4,6})",
            text,
            re.IGNORECASE,
        )
        if debit_match:
        ...
        # Pattern 2: "A/C {acct} debited for Rs.{amt}"
        debit_alt = re.search(
            r"(?:A/C|a/c|Account)\s*(?:No\.?\s*)?([*Xx]*\d{4,6})\s+(?:is|has been)\s+debited\s+(?:for|with)\s+(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)",
            text,
            re.IGNORECASE,
        )
        if debit_alt:
        ...
        credit_match = re.search(
            r"(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+has\s+been\s+credited\s+to\s+(?:A/C|a/c|Account)\s*(?:No\.?\s*)?([*Xx]*\d{4,6})",
            text,
            re.IGNORECASE,
        )
        if credit_match:
    ...
    def parse_pdf(
        ...
        """Parse Axis Bank bank account PDF statement."""
