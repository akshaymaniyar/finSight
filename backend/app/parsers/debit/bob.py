...
class BOBParser(BaseBankParser):
    ...
    sender_emails = ["alerts@bankofbaroda.com"]
    ...
    def parse_email(
        ...
        # Pattern 1: "Your BOB A/c {acct} is debited by Rs.{amt}"
        debit_match = re.search(
            r"(?:Your\s+)?(?:BOB\s+)?(?:A/[Cc]|Account)\s*([*Xx]*\d{4,6})\s+(?:is|has been)\s+debited\s+(?:by|with|for)\s+(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)",
            text,
            re.IGNORECASE,
        )
        if debit_match:
        ...
        debit_alt = re.search(
            r"(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+(?:has been\s+)?debited\s+from\s+(?:your\s+)?(?:BOB\s+)?(?:A/[Cc]|Account)\s*([*Xx]*\d{4,6})",
            text,
            re.IGNORECASE,
        )
        if debit_alt:
        ...
        credit_match = re.search(
            r"(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+(?:has been\s+)?credited\s+to\s+(?:your\s+)?(?:BOB\s+)?(?:A/[Cc]|Account)\s*([*Xx]*\d{4,6})",
            text,
            re.IGNORECASE,
        )
        if credit_match:
    ...
    def parse_pdf(
        ...
        """Parse Bank of Baroda bank account PDF statement."""
