...
class KotakParser(BaseBankParser):
    ...
    sender_emails = ["noreply@kotak.com", "alerts@kotak.com"]
    ...
    def parse_email(
        ...
        # Pattern 1: "Your A/c XX1234 is debited for Rs.{amt}"
        debit_match = re.search(
            r"(?:Your\s+)?(?:A/[Cc]|Account)\s*([*Xx]+\d{4,6})\s+(?:is|has been)\s+debited\s+(?:for|with|by)\s+(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)",
            text,
            re.IGNORECASE,
        )
        if debit_match:
        ...
        # Pattern 2: "Rs.{amt} debited from A/c XX1234"
        debit_alt = re.search(
            r"(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+(?:has been\s+)?debited\s+from\s+(?:A/[Cc]|Account)\s*([*Xx]*\d{4,6})",
            text,
            re.IGNORECASE,
        )
        if debit_alt:
        ...
        credit_match = re.search(
            r"(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+(?:has been\s+)?credited\s+to\s+(?:A/[Cc]|Account)\s*([*Xx]*\d{4,6})",
            text,
            re.IGNORECASE,
        )
        if credit_match:
    ...
    def parse_pdf(
        ...
        """Parse Kotak Mahindra Bank bank account PDF statement."""
