...
class KotakCCParser(BaseBankParser):
    bank_name = "Kotak Mahindra Credit Card"
    sender_emails = [
        "creditcard@kotak.com",
    ...
    def parse_email(
        ...
        # Pattern 1: "Rs.{amt} spent on your Kotak Credit Card ending {card} at {merchant}"
        spend_match = re.search(
            r"(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+(?:has been\s+)?spent\s+on\s+(?:your\s+)?(?:Kotak\s+(?:Mahindra\s+)?)?[Cc]redit\s+[Cc]ard\s+(?:ending\s+)?(\d{4})\s+at\s+([A-Za-z0-9\s&.\-/]+?)(?:\s+on\s+|\s*\.\s*|$)",
            text,
            re.IGNORECASE,
        )
        if spend_match:
        ...
        # Pattern 2: "Transaction of Rs.{amt} on Kotak Credit Card ending {card}"
        txn_match = re.search(
            r"[Tt]ransaction\s+of\s+(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+(?:on|using)\s+(?:your\s+)?(?:Kotak\s+(?:Mahindra\s+)?)?[Cc]redit\s+[Cc]ard\s+(?:ending\s+)?(\d{4})",
            text,
            re.IGNORECASE,
        )
        if txn_match:
        ...
        # Pattern 3: "Kotak Credit Card ending {card} used for Rs.{amt} at {merchant}"
        used_match = re.search(
            r"(?:Kotak\s+(?:Mahindra\s+)?)?[Cc]redit\s+[Cc]ard\s+(?:ending\s+)?(\d{4})\s+(?:has been\s+)?used\s+for\s+(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)",
            text,
            re.IGNORECASE,
        )
        if used_match:
        ...
        payment_match = re.search(
            r"(?:payment|refund)\s+of\s+(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+(?:has been\s+)?(?:received|credited)\s+(?:on|to)\s+(?:your\s+)?(?:Kotak\s+(?:Mahindra\s+)?)?[Cc]redit\s+[Cc]ard\s+(?:ending\s+)?(\d{4})",
            text,
            re.IGNORECASE,
        )
        if payment_match:
