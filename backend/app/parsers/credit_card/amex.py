...
class AmexParser(BaseBankParser):
    ...
    def parse_email(
        ...
        # Pattern 1: "A charge of Rs.{amt} was made on your American Express Card ending {card} at {merchant}"
        charge_match = re.search(
            r"(?:charge|transaction)\s+of\s+(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+(?:was\s+)?(?:made|processed)\s+on\s+(?:your\s+)?(?:American\s+Express\s+|Amex\s+)?[Cc]ard\s+(?:ending\s+)?(\d{4,5})",
            text,
            re.IGNORECASE,
        )
        if charge_match:
            amount_str = charge_match.group(1).replace(",", "")
            card = charge_match.group(2)
            merchant = ""
            merch_match = re.search(
                r"(?:at|to|towards)\s+([A-Za-z0-9\s&.\-/]+?)(?:\s+on\s+|\s*\.\s*|$)",
                text[charge_match.end() :],
                re.IGNORECASE,
            )
            if merch_match:
        ...
        # Pattern 2: "Rs.{amt} spent on your Amex Card ending {card}"
        spend_match = re.search(
            r"(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+(?:has been\s+)?spent\s+on\s+(?:your\s+)?(?:American\s+Express\s+|Amex\s+)?[Cc]ard\s+(?:ending\s+)?(\d{4,5})",
            text,
            re.IGNORECASE,
        )
        if spend_match:
        ...
        # Pattern 3: "Your Amex Card ending {card} was used for Rs.{amt} at {merchant}"
        used_match = re.search(
            r"(?:American\s+Express\s+|Amex\s+)?[Cc]ard\s+(?:ending\s+)?(\d{4,5})\s+(?:was|has been)\s+used\s+for\s+(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+at\s+([A-Za-z0-9\s&.\-/]+?)(?:\s+on\s+|\s*\.\s*|$)",
            text,
            re.IGNORECASE,
        )
        if used_match:
        ...
        payment_match = re.search(
            r"(?:payment|refund)\s+of\s+(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d{1,2})?)\s+(?:has been\s+)?(?:received|credited|applied)\s+(?:on|to)\s+(?:your\s+)?(?:American\s+Express\s+|Amex\s+)?[Cc]ard\s+(?:ending\s+)?(\d{4,5})",
            text,
            re.IGNORECASE,
        )
        if payment_match:
